...
ActionSet::ActionSet(OpenXRApi *p_api, const String &p_name, const String &p_localised_name, uint32_t p_priority) {
	xr_api = p_api;
	name = p_name;
...
#ifdef DEBUG
	print_line(vformat("Creating action set %s - %s %s", actionSetInfo.actionSetName, actionSetInfo.localizedActionSetName, actionSetInfo.priority));
#endif
...
String ActionSet::get_name() const {
	return name;
}

Action *ActionSet::add_action(XrActionType p_type, const String &p_name, const String &p_localised_name, int p_toplevel_path_count, const XrPath *p_toplevel_paths) {
	if (is_attached) {
...
Action *ActionSet::get_action(const String &p_name) {
	// find this action
	for (uint64_t i = 0; i < actions.size(); i++) {
		if (actions[i]->get_name() == p_name) {
			return actions[i];
...
bool ActionSet::attach() {
	...
	if (handle == XR_NULL_HANDLE) {
		ERR_PRINT("Can't attach action set if it has not been created.");
		return false;
	}
	if (xr_api->session == NULL) {
		ERR_PRINT("Can't attach action set if there is no session.");
		return false;
	}
