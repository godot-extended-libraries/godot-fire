...
public:
	Action(OpenXRApi *p_api, XrActionSet p_action_set, XrActionType p_type, const String &p_name, const String &p_localised_name, int p_toplevel_path_count, const XrPath *p_toplevel_paths);
	~Action();
	...
	Vector2 get_as_vector(const XrPath p_path);
	bool is_pose_active(const XrPath p_path);
	Transform get_as_pose(const XrPath p_path, float p_world_scale);
