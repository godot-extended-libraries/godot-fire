...
void OpenXRHand::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("_physics_process"), &OpenXRHand::_physics_process);

	ClassDB::bind_method(D_METHOD("get_hand"), &OpenXRHand::get_hand);
	ClassDB::bind_method(D_METHOD("set_hand"), &OpenXRHand::set_hand);
...
void OpenXRHand::_ready() {
	...
	// We cast to node 3ds which should allow us to use any subclass of that.
	for (int i = 0; i < XR_HAND_JOINT_COUNT_EXT; i++) {
		String node_path = node_names[i];
		joints[i] = Object::cast_to<Node3D>(get_node(node_path));
		if (joints[i] == NULL) {
