...
Action::Action(OpenXRApi *p_api, XrActionSet p_action_set, XrActionType p_type, const String &p_name, const String &p_localised_name, int p_toplevel_path_count, const XrPath *p_toplevel_paths) {
	xr_api = p_api;
	type = p_type;
	name = p_name;
...
#ifdef DEBUG
	print_line(vformat("Created action %s %s %s", actionInfo.actionName, actionInfo.localizedActionName, actionInfo.countSubactionPaths));
#endif
...
#ifdef DEBUG
			print_line(vformat("Created space for %s/%s", name, i));
#endif
		}
	}
}
...
bool Action::is_pose_active(XrPath p_path) {
	if (handle == XR_NULL_HANDLE || p_path == XR_NULL_PATH) {
		// not initialised
		print_line("Pose not initialised");
		return false;
	} else if (type != XR_ACTION_TYPE_POSE_INPUT) {
		// wrong type
		print_line("Not a pose type");
		return false;
	} else {
		XrActionStateGetInfo getInfo = {
			.type = XR_TYPE_ACTION_STATE_GET_INFO,
	...
	if (handle == XR_NULL_HANDLE || p_path == XR_NULL_PATH) {
		...
		if (spaceLocationValid != toplevel_paths[index].wasLocationvalid) {
			...
			if (!spaceLocationValid) {
				ERR_PRINT(String("OpenXR Space location not valid for hand ") + String::num_int64(index));
				return Transform();
			} else {
				print_line(vformat("OpenXR regained tracking for hand %d", index));
			}
		}
